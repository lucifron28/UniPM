...
| IMP-013 | implementation | React browser authentication integration | reviewed | source-inspected | `aaad0b8` | [record](implementation/IMP-013-web-browser-authentication.md) | Memory-only access token, refresh-cookie restoration, bounded replay, Query-owned current user, and protected routes. |
| ADR-009 | decision | Coordinate memory-only browser sessions with bounded refresh replay | reviewed | source-inspected | `aaad0b8` | [record](decisions/ADR-009-browser-session-restoration-and-refresh-retry.md) | Single-flight refresh, one replay, generation races, and complete Query clearing. |
| TEST-017 | test-run | React browser authentication verification | executed | locally-executed | `aaad0b8` | [record](test-runs/TEST-017-web-authentication-verification.md) | npm 10/11 clean installs, 58 Vitest tests, 10 Chromium tests, and the 280-pass backend Release baseline passed. |
