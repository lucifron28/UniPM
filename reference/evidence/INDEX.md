...
| IMP-013 | implementation | React browser authentication integration | reviewed | source-inspected | `8b2f143` | [record](implementation/IMP-013-web-browser-authentication.md) | Memory-only access token, generation-bound refresh, serialized cookie transitions, bounded replay, and protected routes. |
| ADR-009 | decision | Coordinate memory-only browser sessions with bounded refresh replay | reviewed | source-inspected | `8b2f143` | [record](decisions/ADR-009-browser-session-restoration-and-refresh-retry.md) | One refresh flight per generation, ordered cookie writers, one replay, and complete Query clearing. |
| TEST-017 | test-run | React browser authentication verification | executed | locally-executed | `8b2f143` | [record](test-runs/TEST-017-web-authentication-verification.md) | npm clean install, 59 Vitest tests, 11 Chromium tests including actual cookie ordering, and the 280-pass backend Release baseline passed. |
