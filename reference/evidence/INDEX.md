...
| IMP-015 | implementation | React web preventive-maintenance schedules | reviewed | source-inspected | `f6eadd2` | [record](implementation/IMP-015-react-web-preventive-maintenance-schedules.md) | Typed list/detail/create workflow over recorded schedules without recurrence or automatic status behavior. |
| ADR-011 | decision | Keep schedule monitoring record-based and non-generative | reviewed | source-inspected | `f6eadd2` | [record](decisions/ADR-011-record-based-schedule-monitoring.md) | Router/Query/catalog ownership with recorded-status and provisional authority boundaries. |
| TEST-019 | test-run | Web schedule workflow verification | executed | locally-executed | `f6eadd2` | [record](test-runs/TEST-019-web-schedule-workflow-verification.md) | Release backend, offline API contract, 103 Vitest, coverage, and 30 Chromium tests passed; SQL/provider tests not executed. |
